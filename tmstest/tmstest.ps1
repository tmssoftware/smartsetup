...
foreach ($test in $tests) {
    ...
    try {
       ...
       try
       {
          ...
          Start-Transcript -Path "$testDir\output.log" | Out-Null
          try {
          & $test.FullName | Out-Null  # *>&1 | Out-File -FilePath "$testDir\output.log" -Encoding utf8            
          }
          finally {
        ...
        if ($tests.Length -eq 1) {Write-Host "Error executing test script: $($_.Exception.Message)"}
...
Write-Host "Skipped tests: $($skippedTests.Count)" -ForegroundColor Yellow
