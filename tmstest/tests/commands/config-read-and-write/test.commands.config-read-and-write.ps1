...
function Test-ValuesOk() {
    ...
    for ($i = 0; $i -lt $productArray.Count; $i++) {
        ...
        tms config-read "configuration for $($productArray[$i]):compiler parameters:delphi12" | Assert-ValueIs $productCompilerParametersD12[$i]
    }
}
...
$productCompilationOptionsDefines = $("[DEBUG]", "[]", "[DEBUGX,TESTX= false]")
for ($i = 0; $i -lt $productArray.Count; $i++) {
...
$productCompilationOptionsDefines = $("[DEBUG]", "[]", "[DEBUGO,TESTO: false]")
for ($i = 0; $i -lt $productArray.Count; $i++) {
    tms config-write -p:"configuration for $($productArray[$i]):compilation options:defines=$($productCompilationOptionsDefines[$i])"
}
$productCompilationOptionsDefines = $("[DEBUG]", "", "[DEBUGO,TESTO: false]")
...
$productCompilerParametersD12 = $("-p:one", "-p:two -p:three", "")
for ($i = 0; $i -lt $productArray.Count; $i++) {
    tms config-write -p:"configuration for $($productArray[$i]):compiler parameters:delphi12=$($productCompilerParametersD12[$i])"
}
Test-ValuesOk

Test-Json -fileName "tms.config2"

tms config-read -cmd | Out-File -FilePath "tms.config.cmd" -Encoding utf8
#rename tms.config.yaml to tms.config.final.yaml, so we can check it later
Rename-Item -Path "tms.config.yaml" -NewName "tms.config.final.yaml" -Force
tms config -print

$def = Get-Content "tms.config.cmd"
foreach ($line in $def) {
    $escapedLine = $line.Substring(4, $line.Length - 5)  #remove the leading -p:
    tms config-write -p:$escapedLine
}

#compare the tms.config.yaml generated by the config-write with the one generated by config -print, they should be the same
compare-files "tms.config.final.yaml" "tms.config.yaml"
