...
type
  ...
  public
    ...
    //A little faster than calling Dest.CopyFrom(ThisStream), as it doesn't double-copy the memory.
    procedure SaveToStream(const Dest: TStream; ACount: integer);
  private
    FSource: TStream;
...
const
  ...
  ERROR_INVALID_FUNCTION = 38; //ENOSYS
{$ENDIF}
{$IFDEF MACOS}
const
  ERROR_INVALID_PARAMETER = 100000 + 22; //EINVAL
  ERROR_INVALID_FUNCTION = 100000 + 38; //ENOSYS
...
procedure TZSTDDecompressStream.SaveToStream(const Dest: TStream; ACount: integer);
begin
  if ACount = 0 then Exit;
  ...
  while ACount > 0 do
    begin
      var AvailableCount := FOutput.pos - FStreamOutBufferSizePos;
      if Integer(AvailableCount) > ACount then
        AvailableCount := ACount;
      if AvailableCount > 0 then
        begin
          var Source: PByte := FStreamOutBuffer;
          Inc(Source, FStreamOutBufferSizePos);
          Dest.WriteBuffer(Source^, AvailableCount);
    ...
    begin
      ...
      if AvailableCount > 0 then
        begin
          ...
          Inc(Source, FStreamOutBufferSizePos);
          System.Move(Source^, Buffer^, AvailableCount);
          Inc(FStreamOutBufferSizePos, AvailableCount);
