...
{{#Important}}
The information in this section is for advanced use; you will probably not need it. Most times, you will have a single configuration (or even no configuration), so the inheritance rules don't matter as there is nothing to inherit. 
...
But when the values are an array of items, things get a little more complex. When there is a new array in the next source, should it completely replace the old values, or add them to the previous configuration? The answer is not always clear.
...
And then you override the values in `tms.config.yaml`:
...
configuration for all products:
  ...
  compilation options:
    defines:
      - RTTI
```
Here, if we replace the old values completely, we will end up compiling for all Delphi versions (because `delphi versions` is empty), and we will only pass the **RTTI** define, not the **UNICODE** defined in the base configuration. This is not likely what you want. But at other times, it might be exactly what you want. If you had the following in `tms.config.yaml`:
...
Given that there is no clear best default behavior, we opted not to provide any. If you are inheriting values from a previous configuration (which will not be a normal case), you need to tell Smart Setup explicitly what you want to do with the arrays. For all array properties, you have 3 options (shown below with the "delphi versions" property to make it easier to follow):
...
configuration for all products:
  delphi versions:
    -delphi13
```
It will only work if the previous array is empty. If there is a previous value in the array, it will throw an exception telling you to use either of the two following options.
...
We recommend you use **default** properties. When and if there is a conflict between an array and its previous value, Smart Setup will tell you and let you fix it. When fixing it, you can think exactly about whether you want to add or replace the values for that particular case. 
...
```
This is done mostly not to break existing scripts. But when writing new scripts, it is better that you explicitly say if you want to add to or replace the values in the array property, by **using only "add-" and "replace-" prefixes**
{{/Important}}
